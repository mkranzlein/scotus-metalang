...
bert_tokenizer = transformers.BertTokenizerFast.from_pretrained('bert-base-uncased', do_lower_case=True)
bert_model = transformers.BertModel.from_pretrained("bert-base-uncased").cuda()

device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
print("Using device:", device)

BATCH_SIZE = 8  # Number of sentences per batch
EPOCHS = 8
LEARNING_RATE = 1e-4
...
for json_doc in json_data:
    ...
    for raw_sent in json_doc["sentences"]:
        ...
        token_labels = datasets.get_multilabel(raw_sent, curiam_categories.REDUCED_CATEGORIES)
...
model = btm.BinaryTokenModel(num_labels=4, bert_model=bert_model, device=device)
...
for epoch in range(EPOCHS):
    batch_losses = btm.train_loop(model, train_dataloader, optimizer, scheduler)
    print(np.mean(batch_losses))
    btm.eval_loop(model, val_dataloader, device, 4)
